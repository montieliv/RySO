...
<head>
  ...
  <link  href='css/slideSUP.js.css' rel='stylesheet'/>  
  <style type="text/css">
...
<script>
 $(document).ready(function() {
  ...
  document.getElementById("buscando").focus(); 
});
</script>
<div id="obrasTodas">   
        <table cellpadding="0px" cellspacing="0px" border="1px solid black" bordercolor="#EEEEEE" id="tableC">
          <thead>
            <tr>
                ...
                <th align="center" width="30%">Observación</th> 
                <th align="center" width="30%">Alerta</th>
          ...
          <tbody>    
             <?php   
                  ...
                  $numtodos = mysql_query("SELECT * FROM obrasgmaec,gestionobras where  (id_Obras=id_ObrasGestion) and (idsuperSuper=".$_SESSION["idUsuario"].") order by isPrioritaria desc, creada asc",$conec); 
                    $ix=0;             
                     ...
                     {   ?>
                                <tr>                     
                                  ...
                                  <td><?php 
                                            ...
                                            if ($iscroq['0'] > 0) { ?> <a href="#" class="btn btnvalidoC" id="btnvalidoC" onClick="croquisValidado(<?php echo $pp['0'] ?>);">Val.:&nbsp<?php echo $cv['0']." / ".$iscroq['0']; ?></a> <?php } 
                                                    else { ?> <a href="#" class="btn btnvalidaC" id="btnvalidaC" onClick="croquisValidado(<?php echo $pp['0'] ?>);">+ Agregar</a> <?php } ?>  
                                  </td>
                                  ...
                                  <td align="center" <?php if ($obrpre['0'] != 0) { ?> style="background:green;"  <?php } else { ?> style="background:red;"  <?php } ?> >
                                  <p id="<?php echo $pp['id_Obras']."txt"?>"> <?php if ($obrpre['0'] != 0) { echo "<b>$$$ OK</b>"; } else { echo "<b>$ :(</b>"; }?></p>
                                  </td>                                  
                                  ...
                                  <td style="font-size:.9em; mmarging:0; padding:0;"> <?php                                    
                                      ...
                                      if($nI[6]=="Solicitud"){$fechaAcuseO=$nI[5];} //de aquí se toma la fecha para calcular los días restantes!                                        
                                  ...
                                  <td> 
                                      <?php  $obtenergestor = mysql_query("select nomber from gestores where (id_Gestor=".$pp['idGesa'].")",$conec); 
                                         $inc=0;
                                  ...
                                  <td> <spam style="margin:0; padding:0; cursor:pointer;" id="<?php echo $pp['id_Obras'] ?>" OnClick="verPermisosTotales(this,<?php echo $pp['pObtenidos']+$pp['pPendientes']; ?>);"> <?php echo $pp['pObtenidos']+$pp['pPendientes']." = ";?></spam>
                                  ...
                                  <td>
<?php 
                                          //90 días: vencido=-, rojo=0-17, amarillo=18-53, verde=54-100%
                                                             //<=19%       >=20% y <=59%       >60%      
                                         //30 días: vencido=-, rojo=0-5, amarillo=6-17, verde=18-100%
 ?>                                    
                                <?php                                

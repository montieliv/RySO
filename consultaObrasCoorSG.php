...
<head>
	<meta charset="utf-8"/>
	<title>Obras</title> <!-- Consulta del Coor sin gestor -->
...
<script>
 $(document).ready(function() {
	oTableSG = $('#dataTableCoorsg').dataTable({
...
<div id="obrasTodas">  	
				<table cellpadding="0px" cellspacing="0px" border="1px solid black" bordercolor="#EEEEEE" id="dataTableCoorsg">
					 <thead>
            <tr>
          ...
          <tbody>    
             <?php  
                  ...
                  while( $pp = mysql_fetch_array($numtodos) )
                     {  $obtenergestor = mysql_query("select nomber,supervisor,idGesa from gestores,supervisionobras where ((idGesa=id_Gestor) and (idObraa=".$pp['id_Obras']."))",$conec); 
                         $nI2 = mysql_fetch_array($obtenergestor);
                       ...
                       ?>                               
                                <?php if (($nI2['2'] == 28) and ($iscroq['0'] > 0) and ( $cv[0]==$iscroq[0])) { ?>
                                <tr>                                       

...
<html lang="es">
<head>
	<meta charset="utf-8"/> <!-- Consulta del Jefe de Proyecto -->
	<title>Obras</title>
...
<div id="obrasTodas">   
        <table cellpadding="0px" cellspacing="0px" border="1px solid black" bordercolor="#EEEEEE" id="tableJ">
          <thead>
            <tr>
                <th align="center" width="7%">Status</th>
                <th align="center" width="2%" style="font-size:.6em; background:gold; color:blue;">PRIORITARIA</th>
          ...
          <tbody>    
             <?php  
                  ...
                  $numtodos = mysql_query("SELECT * FROM obrasgmaec,gestionobras where  (id_Obras=id_ObrasGestion) and (id_J=".$_SESSION["idUsuario"].") order by isPrioritaria desc, creada asc",$conec); 
                  $ix=0;                             
                  while( $pp = mysql_fetch_array($numtodos) )
                     { ?>
                                <tr>                     
                                  <td> <?php echo $pp['status'] ?> </td>
                                  <td align="center"> <?php if ($pp['isPrioritaria']=="true"){ ?> <span class="icon-star-empty"><p style="font-size:0em;">pri</p></span>  <?php } else {echo "-";} ?> </td>
                                  <td> <?php echo $pp['tipoObra']; ?></td>
                                  <td><?php 
                                  ...
                                  while( $nI = mysql_fetch_array($numI)) {                                             
                                      ...
                                      if ($nI[6]=='Permiso') {echo "#:".$nI[1]."<br>";  } else {echo "#:".$nI[1]."<br>";  }

...
<head>
	<meta charset="utf-8"/>
	<title>Obras</title>  <!-- Consulta del Administrativo-->
...
<script>
  ...
  function verPresup(id){           
   $.get("activapresup.php?idP="+id.value,function(data){
   var txt2 = data;   
   var caj= document.getElementById(id.value+"ok");     
   if (data !='Activado') {  caj.style.background = "red";    document.getElementById(id.value+"txt").innerHTML = "<strong>$ :(</strong>";
  } 
    else { caj.style.background = "green";   document.getElementById(id.value+"txt").innerHTML = "<strong>$$$ OK</strong>";
 }     
  });    
}
</script>
<div id="obrasTodas">   
        <table cellpadding="0px" cellspacing="0px" border="1px solid black" bordercolor="#EEEEEE" id="tableAD">
          <thead>
            <tr>
                <th align="center" width="8%">Presupuesto</th>
                <th align="center" width="10%">Status</th>
                <th align="center" width="10%">Tipo<br>Obra</th>                         
                  ...
                  while( $pp = mysql_fetch_array($numtodos) )
                     { ?>
                                ...
                                <?php $obrpre=mysql_fetch_array(mysql_query("Select COUNT(id_presupuesto) From presupuestoobras Where id_ObraP=".$pp['id_Obras'],$conec));  ?>              
                                  <td align="center" id="<?php echo $pp['id_Obras']."ok"?>"   <?php if ($obrpre['0'] == 0) { ?> style="background:red; align:center;" <?php } else { ?> style=" background:green;" <?php } ?> >                                                                           
                                        <input type="checkbox"   value="<?php echo $pp['id_Obras']?>" OnClick="verPresup(this);" <?php if ($obrpre['0'] != 0) { ?> checked  <?php } ?>> <p id="<?php echo $pp['id_Obras']."txt"?>"> <?php if ($obrpre['0'] != 0) { echo "<b>$$$ OK</b>"; } else { echo "<b>$ :(</b>"; }?></p>
